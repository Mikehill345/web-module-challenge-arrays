...
let originalCopy = [...originalFlavors];
...
function is31Flavors(originalCopy) {
    return originalCopy.length === 31
}
//  console.log(is31Flavors(originalCopy));
...
function addFlavor(newFlavor, orignialCopy) {
    originalCopy.unshift(newFlavor)
    console.log(originalCopy)
}
// addFlavor('Rainbow Sherbert', originalCopy)
...
function removeLastFlavor(originalCopy) {
    originalCopy.pop()
    console.log(originalCopy)
}
// console.log(removeLastFlavor(originalCopy));
...
function getFlavorByIndex(originalCopy, flavorIndex) {
    return originalCopy[flavorIndex]
}

// console.log(getFlavorByIndex(originalCopy, 2))
...
function removeFlavorByName(originalCopy, nameOfFlavor) {
    for (let i = 0; i < originalCopy.length; i++) {
        if (originalCopy[i] === nameOfFlavor) {
            originalCopy.splice(i, 1);
        }
    }
    return originalCopy;
}
//    console.log(removeFlavorByName(originalCopy, 'Vanilla'))
...
function filterByWord(originalCopy, flavor) {
    let newArr = [];
    for (let i = 0; i < originalCopy.length; i++) {

        if (originalCopy[i].includes(flavor)) {
            newArr.push(originalCopy[i]);
        }
    }
    return newArr;
}
// console.log(filterByWord(originalCopy, 'Chocolate'))
...
function getAverageWordLength(originalCopy) {
    let count = 0;
        for(let i = 0; i < originalCopy.length; i++){
      count += originalCopy[i].split(' ').length
    }   
    return count / originalCopy.length;
}
console.log(getAverageWordLength(originalCopy));
